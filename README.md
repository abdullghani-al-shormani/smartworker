...
* 🟢 Java & Kotlin Support: First-class experience for both languages.
...
**kotlin**
```kotlin
val workId: UUID = SmartWorker.with(this)
    .oneTime(MySyncWorker::class.java)
    .requiresInternet()             // Require active internet connection
    .requiresCharging()             // Require device to be plugged in
    .setDelay(5, TimeUnit.MINUTES)  // Delay execution by 5 minutes
    .enqueue()
...
```kotlin
SmartWorker.with(this)
    .periodic(DatabaseBackupWorker::class.java, 2, TimeUnit.HOURS)
    .requiresStorageNotLow()    // Require sufficient storage space
    .requiresBatteryNotLow()    // Require battery not to be low
    .setExistingPolicy(ExistingPeriodicWorkPolicy.KEEP) // Keep existing task if present
    .enqueueUnique("daily_db_backup")
...
**kotlin**
```kotlin
val user = UserProfile("Abdullghani", "Admin")
SmartWorker.with(this)
    .oneTime(UpdateProfileWorker::class.java)
    .putString("user_id", "123")
    .putInt("retry_count", 3)
    .putObject("user_profile", user) // Pass custom object directly
    .enqueue()
...
class UpdateProfileWorker(
    ...
    override fun doWork(): Result {
        // Retrieve standard primitive data
        val userId: String? = inputData.getString("user_id")

        // Effortlessly retrieve the custom object
        val user: UserProfile? = SmartWorkerHelper.getObject(
            inputData, "user_profile", UserProfile::class.java
        )
...
SmartWorker.chain(this)
    .beginParallel(
        DownloadImageWorker::class.java,
        DownloadAudioWorker::class.java
    ) // Execute in parallel
    .then(ProcessDataWorker::class.java)    // Execute after downloads complete
    .then(UploadServerWorker::class.java)   // Final execution step
    .start()
```
...
**kotlin**
```kotlin
SmartWorker.observe(this, context, "daily_db_backup", object : WorkStateListener {

    override fun onSuccess(outputData: Data) {
        // Task completed successfully
    }

    override fun onFailure(outputData: Data) {
        // Task failed
    }

    override fun onProgress(progressData: Data) {
        // Extract real-time progress data
        val progress = progressData.getInt("progress_key", 0)
    }
})
```
...
```kotlin
SmartWorker.with(context)
    .oneTime(FileDownloadWorker::class.java)
    .asForeground(NOTIFICATION_ID, notificationObject)
    .enqueue()
```
